...
class PhotoGestionAppLifecycleTest {
    ...
    void shouldShutdownThumbnailExecutorWhenStageCloses() throws Exception {
        ...
        assertTrue(stageClosed.await(5, TimeUnit.SECONDS), "La fermeture de la fenetre doit etre traitee");
        ...
        assertTrue(waitForNoThumbnailThreads(2, TimeUnit.SECONDS),
                "Le pool de miniatures doit etre arrete apres fermeture du stage");
    }

    private boolean waitForNoThumbnailThreads(long timeout, TimeUnit unit) throws InterruptedException {
        long deadline = System.nanoTime() + unit.toNanos(timeout);
        while (System.nanoTime() < deadline) {
            boolean none = Thread.getAllStackTraces().keySet().stream()
